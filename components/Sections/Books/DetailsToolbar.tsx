...
// --- 🔘 Elite Dropdown (Performance Optimized) ---
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button 
                ...
                className={`h-11 px-4 rounded-2xl bg-[var(--bg-card)] border border-[var(--border)] flex items-center gap-3 text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 ${isOpen ? 'border-orange-500 shadow-lg' : 'hover:border-orange-500/20'}`}
            >
                ...
                <span className="hidden lg:block text-[var(--text-main)] truncate max-w-[100px]">
                     {t(`category_${current.toLowerCase()}`) || current}
                </span>
                <ChevronDown size={12} className={`hidden lg:block opacity-30 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div 
                        ...
                        exit={{ opacity: 0, y: 5 }} 
                        className="absolute right-0 mt-3 w-56 bg-[var(--bg-card)] backdrop-blur-3xl border border-[var(--border)] rounded-[28px] p-2 z-[1000] shadow-2xl"
                    >
                        ...
                        <div className="max-h-[250px] overflow-y-auto no-scrollbar py-1">
                            {options.map((opt: string) => {
                                ...
                                return (
                                    ...
                                    >
                                        {t(`category_${opt.toLowerCase()}`) || opt} 
...
export const DetailsToolbar = ({ 
    ...
    // --- 🚀 ল্যাগ-ফ্রি ট্রানজিশন কনফিগ ( tween is better for low-end phones ) ---
    ...
    return (
        <div className="sticky top-[4.5rem] md:top-20 z-[300] bg-[var(--bg-app)]/80 backdrop-blur-xl px-1 py-4 mb-4 border-b border-transparent hover:border-[var(--border)] transition-all duration-500">
            <div className="flex items-center gap-3 h-12 relative w-full">
                
                {/* --- ১. স্মার্ট সার্চ এরিয়া (Zero Jitter Layout) --- */}
                <motion.div 
                    layout
                ...
                >
                    ...
                    <input 
                        ...
                        className="w-full h-11 bg-[var(--bg-card)] border border-[var(--border)] rounded-[22px] pl-12 pr-12 text-[11px] font-bold uppercase tracking-widest focus:border-orange-500/40 outline-none transition-all shadow-inner text-[var(--text-main)] placeholder:opacity-20"
                    />
                ...
                {/* --- ২. ফিল্টার বাটনসমূহ (মোবাইলে সার্চের সময় ডিসপ্লে হাইড) --- */}
                <motion.div 
                    layout
                    transition={fastTransition}
