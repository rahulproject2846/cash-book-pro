...
// --- ১. ইউনিফাইড কাস্টম ড্রপডাউন (Theme & Mobile Optimized) ---
    ...
    return (
        <div className="relative w-full md:w-auto" ref={dropdownRef}>
            ...
            >
                ...
                <span className="truncate">{value === 'All' ? 'VIEW ALL' : value.toUpperCase()}</span>
                    ...
                    >
                        {options.map((opt: string) => (
                            <button
                                ...
                                className={`w-full text-left px-5 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-orange-500/10 hover:text-orange-500 transition-colors flex items-center justify-between ${value === opt ? 'text-orange-500 bg-orange-500/5' : 'text-[var(--text-muted)]'}`}
                            >
                                {opt}
...
export const BookDetails = ({ 
    ...
    const handleShareToggle = async () => {
        ...
        try {
            ...
            if (res.ok) { setIsSharing(data.isPublic); setShareToken(data.shareToken); toast.success("Share Protocol Updated"); }
    ...
    if (categoryFilter !== 'All') processedItems = processedItems.filter(item => item.category === categoryFilter);
...
<div className="flex gap-3 items-center w-full relative z-[50]">
    ...
    <div className="relative flex-1 group">
                        ...
                        <input 
                            ...
                            className="w-full pl-14 pr-6 py-4 bg-[var(--bg-card)] border-2 border-[var(--border-color)] rounded-[20px] text-xs font-black uppercase tracking-widest focus:outline-none focus:border-orange-500 text-[var(--text-main)] shadow-sm transition-all"
                        />
                    </div>

    {/* Desktop View: 4 Buttons (ORDER: VIEW -> SORT -> STATS -> EXPORT) */}
    <div className="hidden xl:flex gap-3">
        
        {/* 1. View/Category Button */}
        ...
        {/* ৩. STATS Button (Direct Local Call) */}
        <button onClick={onOpenAnalytics} className="bordered-icon-btn relative w-full flex cursor-pointer items-center justify-center gap-3 px-6 py-4 rounded-[20px] border-2 border-[var(--border-color)] bg-[var(--bg-card)] text-[var(--text-muted)] text-[11px] font-black uppercase tracking-widest hover:border-orange-500 hover:text-orange-500 hover:shadow-lg transition-all active:scale-95 whitespace-nowrap"><BarChart3 size={18} className="shrink-0" /> <span>STATS</span></button>
                        <button onClick={onOpenExport} className=" relative w-full flex cursor-pointer items-center justify-center gap-3 px-6 py-4 rounded-[20px] border-2 border-[var(--border-color)] bg-[var(--bg-card)] text-[var(--text-muted)] text-[11px] font-black uppercase tracking-widest hover:shadow-lg transition-all active:scale-95 whitespace-nowrap hover:border-green-500 hover:text-green-500"><Download size={18} className="shrink-0" /> <span>EXPORT</span></button>
    </div>

    {/* Mobile View: Settings Button (Direct Local Call) */}
    <div className="xl:hidden relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => setShowMobileSettings(!showMobileSettings)} className="p-4 bg-[var(--bg-card)] border-2 border-[var(--border-color)] rounded-[20px] text-[var(--text-muted)] hover:border-orange-500 active:bg-orange-500 active:text-white transition-all shadow-sm">
            <SlidersHorizontal size={20} />
        </button>
        <AnimatePresence>
            {showMobileSettings && (
                <motion.div initial={{ opacity: 0, scale: 0.9, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="absolute right-0 top-16 w-64 bg-[var(--bg-card)] border-2 border-[var(--border-color)] rounded-[28px] shadow-2xl z-[200] p-6 space-y-6">
                    <div className="space-y-4">
                        ...
                        <div className="border-t border-[var(--border-color)] pt-5 flex gap-4">
                            {/* 🔥 Mobile Analytics & Export (Local Call) */}
                ...
                {/* 4. DATA TABLE (FINAL UX: BALANCED GAPS, NEW NOTE COLUMN & TRUNCATION) */}
<div className="bg-[var(--bg-card)] rounded-[28px] border border-[var(--border-color)] overflow-hidden shadow-sm min-h-[550px] relative transition-all">
    
    {/* --- DESKTOP VIEW --- */}
    <div className="hidden xl:block overflow-x-auto no-scrollbar">
        <table className="w-full text-left border-collapse table-fixed"> {/* table-fixed ensures column widths are maintained */}
            <thead className="bg-[var(--bg-app)]/50 border-b border-[var(--border-color)] sticky top-0 z-10 backdrop-blur-md">
                <tr className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[2px]">
                    ...
                    <th className="py-6 px-4">Note / Memo</th> {/* 🔥 এই কলামটি গ্যাপ ফিল করবে */}
            ...
            <tbody className="divide-y divide-[var(--border-color)]">
                {currentItems.map((e: any, i: number) => (
                    <tr key={e._id} className="hover:bg-[var(--bg-app)]/80 transition-all group">
                        ...
                        <td className="py-6 px-4">
                            <div className="text-sm font-black text-[var(--text-main)] uppercase tracking-tight truncate max-w-[180px]" title={e.title}>
                                {e.title}
                            </div>
                        </td>

                        {/* 🔥 New Note Column (Space Filler) */}
                        <td className="py-6 px-4">
                            <div className="text-[11px] text-[var(--text-muted)] font-medium italic opacity-60 truncate max-w-[250px]" title={e.note}>
                                {e.note ? `"${e.note}"` : "---"}
                            </div>
                        </td>

                        <td className="py-6 px-4">
                            <span className="px-3 py-1.5 rounded-lg bg-orange-500/5 text-orange-500 text-[9px] font-black uppercase border border-orange-500/10 tracking-widest">
                                {e.category}
                            </span>
                        </td>

                        <td className="py-6 px-4 text-center text-[10px] font-bold text-[var(--text-muted)] uppercase opacity-70">
                            {e.paymentMethod}
                        </td>
                        ...
                        <td className="py-6 px-4 text-center">
                            <button onClick={() => onToggleStatus(e)} className={`px-3 py-1 rounded-full text-[8px] font-black uppercase border transition-all ${e.status === 'Completed' ? 'bg-green-500/10 text-green-500 border-green-500/20 shadow-sm' : 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20'}`}>
                                {e.status}
                            </button>
                        </td>
                        ...
                        <td className="py-6 px-6 text-right">
                            <div className="flex justify-end gap-2.5 opacity-0 group-hover:opacity-100 transition-all duration-200">
                                <button onClick={() => onEdit(e)} className="p-2 text-blue-500 bg-blue-500/5 rounded-xl border border-blue-500/10 hover:bg-blue-500 hover:text-white shadow-sm"><Edit2 size={14}/></button>
                                <button onClick={() => onDelete(e)} className="p-2 text-red-500 bg-red-500/5 rounded-xl border border-red-500/10 hover:bg-red-500 hover:text-white shadow-sm"><Trash2 size={14}/></button>
                            </div>
                        </td>
                    </tr>
                ))}
    ...
    {/* --- MOBILE & TABLET VIEW (Adaptive Cards) --- */}
    <div className="xl:hidden p-4 space-y-4">
        {currentItems.map((e: any) => (
             <div key={e._id} className="app-card p-6 flex flex-col gap-4 border-[var(--border-color)] active:scale-[0.98] transition-transform">
                <div className="flex justify-between items-start">
                    <div className="min-w-0 flex-1 pr-4">
                        <h4 className="text-base font-black uppercase text-[var(--text-main)] tracking-tight truncate" title={e.title}>{e.title}</h4>
                ...
                {e.note && (
                    <p className="text-[11px] text-[var(--text-muted)] italic opacity-60 line-clamp-1">"{e.note}"</p>
                )}
        ...
        {currentItems.length === 0 && (
            <div className="py-20 text-center text-slate-500 font-bold uppercase text-xs tracking-widest">
