...
export const StatsGrid = ({ income, expense, labelPrefix = "Total", currency = "৳" }: any) => {
    ...
    // কারেন্সি সিম্বল বের করা (যেমন: "BDT (৳)" থেকে শুধু "৳" নেওয়া)
    const symbol = currency.match(/\(([^)]+)\)/)?.[1] || currency;

    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 anim-fade-up">
            {stats.map((s, i) => (
                <motion.div key={i} whileHover={{ y: -5 }} className={`app-card p-6 border-l-4 ${s.border} ${s.bg}`}>
                    ...
                    <h3 className={`text-2xl font-mono-finance font-bold ${i === 2 ? (s.value >= 0 ? 'text-blue-500' : 'text-red-500') : 'text-[var(--text-main)]'}`}>
                        {s.value >= 0 ? '+' : '-'}{symbol}{Math.abs(s.value).toLocaleString()}
                    </h3>
                </motion.div>
            ))}
        </div>
