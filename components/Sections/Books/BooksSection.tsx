...
export const BooksSection = ({ currentUser }: any) => {
    ...
    const {
        ...
        saveEntry, toggleEntryStatus, togglePin, userId, getBookBalance, setIsMobile, isMobile,

        prefetchNextPage, totalBookCount, allBookIds

    } = useVaultStore();



    // 🛡️ INSTANT SKELETON TRIGGER: Data-driven instead of boot status

    const [showSkeleton, setShowSkeleton] = useState(allBookIds.length === 0 && isRefreshing);
    ...
    // 🌊 DATA-DRIVEN SKELETON LOGIC

    useEffect(() => {

        if (mounted) {

            const shouldShowSkeleton = allBookIds.length === 0 && isRefreshing;

            setShowSkeleton(shouldShowSkeleton);

        }

    }, [allBookIds.length, isRefreshing, mounted]);
    ...
    // 📱 DEVICE DETECTION: Sync mobile state with store
    ...
    // � SMART PAGINATION TRIGGER: Prefetch next page

    useEffect(() => {

        if (bootStatus === 'READY' && !activeBook && !searchQuery) {

            // Calculate total pages based on device and total book count
            ...
            // Trigger prefetch if current page is less than total pages

            if (dashPage < totalPages) {

                prefetchNextPage(dashPage);

            }

        }

    }, [dashPage, bootStatus, activeBook, searchQuery, isMobile, totalBookCount, prefetchNextPage]);
