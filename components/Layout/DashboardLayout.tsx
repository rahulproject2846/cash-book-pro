    ...
    <AnimatePresence>
        {visible && (
            <motion.div 
                ...
                className="absolute -top-10 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-[9px] font-black uppercase px-3 py-1.5 rounded-lg shadow-xl z-[1000] whitespace-nowrap"
            >
                {text}
...
const Sidebar = ({ active, setActive, onLogout, collapsed, setCollapsed, onResetBook, isCompact }: any) => {
    ...
    return (
        ...
        >
            ...
            <div className={`${isCompact ? 'h-16' : 'h-28'} flex items-center ${collapsed ? 'justify-center' : 'pl-8'} border-b border-[var(--border-color)] transition-all`}>
                <div className="flex items-center gap-4">
                    ...
                    <div className={`overflow-hidden transition-all duration-300 ${collapsed ? 'w-0 opacity-0' : 'w-40 opacity-100'}`}>
                        <h1 className="text-xl font-black uppercase italic text-[var(--text-main)] whitespace-nowrap">Vault Pro</h1>
                    </div>
                </div>
            </div>

            <div className={`flex-1 ${isCompact ? 'py-4' : 'py-10'} px-4 space-y-3 overflow-y-auto no-scrollbar`}>
                {NAV_ITEMS.map((item) => {
                    ...
                    return (
                        <button 
                            ...
                            className={`w-full flex items-center ${isCompact ? 'h-11' : 'h-14'} transition-all duration-300 group relative ${collapsed ? 'justify-center rounded-2xl' : 'px-5 rounded-2xl gap-4'} ${isActive ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/25' : 'text-[var(--text-muted)] hover:bg-[var(--bg-app)]'}`}
                        >
...
const BottomNav = ({ active, setActive, onFabClick, onResetBook }: any) => {
    ...
    useEffect(() => {
        ...
        return () => { window.removeEventListener('scroll', handleScroll); clearTimeout(timer); clearTimeout(hideTimer); };
    }, [lastScrollY]);
    ...
    const NavIcon = ({ id, icon: Icon }: any) => {
        ...
        return (
            <button 
                onClick={() => { setActive(id); onResetBook(); }} 
                ...
                >
                    <div className="bg-[var(--bg-card)]/95 backdrop-blur-3xl border border-[var(--border-color)] h-[72px] rounded-[28px] shadow-2xl flex items-center justify-between px-8 relative">
                        ...
                        <div className="relative -top-6">
                            <SmartTooltip text={t('protocol_entry')} visible={showTooltip} />
                            <motion.button 
                                onClick={onFabClick}
...
export const DashboardLayout = (props: DashboardLayoutProps) => {
    ...
    return (
        <div className="flex min-h-screen bg-[var(--bg-app)] text-[var(--text-main)] overflow-x-hidden transition-colors duration-500">
            ...
            <main className={`flex-1 transition-all duration-400 ease-[cubic-bezier(0.25,0.1,0.25,1)] ${collapsed ? 'md:ml-[88px]' : 'md:ml-[280px]'} w-full relative`}>
                <DynamicHeader {...props} collapsed={collapsed} theme={theme} setTheme={setTheme} />

                {/* 🔥 FIX: Removed motion.div key to prevent input re-rendering issue */}
                <div className={`w-full max-w-[1920px] mx-auto transition-all duration-300 ${prefs.compactMode ? 'md:px-6' : 'md:px-10'} ${currentBook && activeSection === 'books' ? 'pt-[120px] px-0' : 'pt-[110px] px-4'} pb-32`}>
                    {children}
                </div>
            </main>
            ...
            <AnimatePresence>
                {isShielded && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[10000] bg-black/60 backdrop-blur-2xl flex flex-col items-center justify-center p-10 text-center text-white">
                        ...
                        <h2 className="text-2xl font-black uppercase italic tracking-tighter">Identity Shield Active</h2>
                        <p className="text-[10px] font-bold text-white/40 uppercase tracking-[3px] mt-2">Protocol locked for your privacy</p>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <style jsx global>{`
                ...
                .compact-deck .app-card { padding: 1rem !important; }
            `}</style>
        </div>
    );
};