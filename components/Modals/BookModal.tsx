...
import { 
    ...
    Plus, Info, Fingerprint, ArrowRight, X, ShieldCheck, Loader2
} from 'lucide-react';
...
export const BookModal = ({ isOpen, onClose, onSubmit, initialData, currentUser }: any) => {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-end md:items-center justify-center overflow-hidden">
            {/* ১. ব্যাকড্রপ: পিওর গ্লাস ইফেক্ট */}
            <motion.div 
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} 
                onClick={onClose} className="fixed inset-0 bg-black/60 backdrop-blur-xl" 
            />

            {/* ২. মডাল কন্টেনার: ড্র্যাগ-টু-ডিসমিস এবং স্প্রিং এনিমেশন */}
            <motion.div 
                ...
                className="bg-[var(--bg-card)] w-full md:max-w-md h-[90vh] md:h-auto rounded-t-[45px] md:rounded-[40px] border-t md:border border-[var(--border)] shadow-2xl relative z-10 flex flex-col overflow-hidden"
            >
                ...
                <div className="px-8 pt-6 pb-2 flex justify-between items-center shrink-0">
                    ...
                    <button onClick={onClose} className="w-10 h-10 rounded-full bg-[var(--bg-input)] flex items-center justify-center text-[var(--text-muted)] hover:text-red-500 transition-all active:scale-90"><X size={20} /></button>
                </div>

                <div className="flex-1 overflow-y-auto no-scrollbar px-8 py-6">
                    <div className="flex flex-col gap-7">
                        {/* ৪. ভিজ্যুয়াল আইডি সিলেকশন */}
                        ...
                        <form className="flex flex-col gap-4" onSubmit={handleAction}>
                            <div className="group relative">
                                ...
                                <input autoFocus={!isMobile} placeholder={form.type === 'general' ? t('placeholder_ledger_name') : t('placeholder_identity_name')} className="w-full h-15 pl-14 bg-[var(--bg-input)] border border-[var(--border)] rounded-[25px] text-[13px] font-bold uppercase outline-none focus:border-orange-500/50 transition-all text-[var(--text-main)]" value={form.name} onChange={e => setForm({...form, name: e.target.value})} required />
                            </div>
                            ...
                            <AnimatePresence mode="wait">
                                {form.type !== 'general' && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="group relative overflow-hidden">
                                        ...
                                        <input type="tel" placeholder="+880 1XXX XXXXXX" className="w-full h-15 pl-14 bg-[var(--bg-input)] border border-[var(--border)] rounded-[25px] text-[13px] font-bold outline-none focus:border-orange-500/50 transition-all text-[var(--text-main)]" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} />
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            <div className="group relative">
                                ...
                                <input placeholder={t('placeholder_vault_memo')} className="w-full h-15 pl-14 bg-[var(--bg-input)] border border-[var(--border)] rounded-[25px] text-[13px] font-bold outline-none focus:border-orange-500/50 transition-all text-[var(--text-main)]" value={form.description} onChange={e => setForm({...form, description: e.target.value})} />
                            </div>

                            {/* সাবমিট বাটন */}
                            <button disabled={isLoading || !form.name} className={`w-full h-16 rounded-[28px] font-black text-[11px] uppercase tracking-[5px] shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3 mt-4 ${form.name ? 'bg-orange-500 text-white shadow-orange-500/30' : 'bg-zinc-800 text-zinc-500 opacity-50'}`}>
