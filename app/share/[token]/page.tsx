...
export default function PublicLedgerPage() {
    ...
    if (loading) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#0F0F0F] transition-colors">
            ...
            <p className="text-[10px] font-black text-slate-400">{t('verifying_protocol')}</p>
        </div>
    );
    ...
    if (error) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#0F0F0F] p-6 text-center transition-colors">
            ...
            <h1 className="text-xl font-black text-white ">{t('access_denied')}</h1>
            <p className="text-sm text-slate-400 mt-2 font-medium max-w-xs mx-auto ">{error}</p>
        </div>
    );
        ...
        <div className="min-h-screen bg-[#0F0F0F] text-white font-sans selection:bg-orange-500/30">
            ...
            <div className="bg-[#1A1A1B] border-b border-[#2D2D2D] pt-12 pb-8 px-4 md:px-8">
                <div className="max-w-5xl mx-auto">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-700/50 border border-slate-700/80 text-slate-300 text-[10px] font-black  mb-4">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                        ...
                        <Tooltip text={t('tt_export_archive')}>
                            <button 
                                ...
                                className="flex items-center gap-2 px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-xl text-[10px] font-black transition-all shadow-xl shadow-orange-600/30 active:scale-95"
                            >
            ...
            <div className="max-w-5xl mx-auto px-4 md:px-8 py-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-[var(--app-gap,1.25rem)]">
                    ...
                    <div className="bg-[#1A1A1B] p-[var(--card-padding,1.75rem)] rounded-2xl border border-[#2D2D2D] shadow-sm flex flex-col justify-between h-36 relative overflow-hidden group">
                        ...
                        <p className="text-[10px] font-black text-slate-400 ">{t('total_inflow')}</p>
                        <h3 className="text-3xl font-mono font-bold text-green-500 ">+{totalIn.toLocaleString()}</h3>
                    </div>
                    ...
                    <div className="bg-[#1A1A1B] p-[var(--card-padding,1.75rem)] rounded-2xl border border-[#2D2D2D] shadow-sm flex flex-col justify-between h-36 relative overflow-hidden group">
                        ...
                        <p className="text-[10px] font-black text-slate-400 ">{t('total_outflow')}</p>
                        <h3 className="text-3xl font-mono font-bold text-red-500 ">-{totalOut.toLocaleString()}</h3>
                    </div>
                    ...
                    <div className="bg-orange-900/20 p-[var(--card-padding,1.75rem)] rounded-2xl border border-orange-900 shadow-2xl flex flex-col justify-between h-36 relative overflow-hidden">
                        ...
                        <p className="text-[10px] font-black text-slate-400 ">{t('net_balance')}</p>
                        <h3 className={`text-3xl font-mono font-bold  ${balance >= 0 ? 'text-blue-400' : 'text-red-400'}`}>
                ...
                <div className="space-y-[var(--app-gap,1.5rem)]">
                    ...
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="w-full pl-14 pr-6 py-5 bg-[#1A1A1B] border border-[#2D2D2D] rounded-[22px] text-xs font-bold text-white placeholder:text-slate-600 focus:outline-none focus:border-orange-500 transition-all shadow-sm"
                            value={searchQuery}
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-[#121212]/50 border-b border-[#2D2D2D]">
                                    <th className="py-5 px-8 text-[10px] font-black text-slate-400">{t('timestamp')}</th>
                                    <th className="py-5 px-6 text-[10px] font-black text-slate-400">{t('transaction_detail')}</th>
                                    <th className="py-5 px-6 text-[10px] font-black text-slate-400">{t('classification')}</th>
                                    <th className="py-5 px-8 text-[10px] font-black text-slate-400 text-right">{t('amount')}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-[#2D2D2D]/50">
                                {filteredEntries.map((e: any) => (
                                    <tr key={e._id} className="hover:bg-[#121212] transition-colors group">
                                        <td className="py-6 px-8 text-xs font-bold text-slate-400 font-mono">
                                            {formatDate(e.date)}
                                        </td>
                                        <td className="py-6 px-6">
                                            <div className="font-black text-sm text-white leading-none">{e.title}</div>
                                            {e.note && <div className="text-[10px] text-slate-400 mt-2 font-medium">"{e.note}"</div>}
                                        </td>
                                        <td className="py-6 px-6">
                                            <span className="px-3 py-1 rounded-lg bg-orange-900/30 text-orange-400 text-[9px] font-black border border-orange-900/50">
                                                {e.category}
                                            </span>
                                        </td>
                            ...
                            <div key={e._id} className="bg-[#1A1A1B] p-6 rounded-[24px] border border-[#2D2D2D] shadow-sm flex flex-col gap-5">
                                <div className="flex justify-between items-start">
                                    <div className="space-y-1">
                                        <h4 className="text-base font-black text-white  leading-none">{e.title}</h4>
                                ...
                                {e.note && <div className="pt-4 border-t border-[#2D2D2D]/50 text-[10px] text-slate-400">"{e.note}"</div>}
                            </div>
                        ))}
                    </div>
                    ...
                    {filteredEntries.length === 0 && (
                        <div className="text-center py-20 bg-[#1A1A1B] rounded-[32px] border border-[#2D2D2D] border-dashed">
                            <p className="text-xs font-black text-slate-600">{t('no_protocol_records')}</p>
                        </div>
                    )}
                </div>
