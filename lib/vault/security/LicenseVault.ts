...
export class LicenseVault {
  ...
  static async verifySignature(user: LocalUser): Promise<boolean> {
    ...
    try {
      ...
      // Create payload string for hashing with timestamp protection

      const timestamp = Math.floor((user.offlineExpiry ?? 0) / (1000 * 60 * 60 * 24)); // Days since epoch

      const payload = `${user._id}:${user.plan}:${user.offlineExpiry}:${timestamp}`;
      ...
      if (!isValid) {
        ...
        // � SECURITY: No detailed logging in production for tampering attempts
      ...
      throw new Error(`CRITICAL_CRYPTOGRAPHIC_ERROR: License signature verification failed: ${error}`);
